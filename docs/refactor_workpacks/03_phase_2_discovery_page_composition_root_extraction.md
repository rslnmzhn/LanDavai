...
- `Related workpacks`: `00`, `23`

## 3. Problem slice

Master plan зафиксировал, что `DiscoveryPage` выступает как UI-hosted composition root. Этот slice выделен отдельно, потому что пока страница создаёт graph сама, никакой честный owner cutover в Discovery невозможен.
...
- `Single write authority after cutover`: app-level composition root for assembly; widgets own только ephemeral UI state
...
- что сейчас является truth:
  - widget constructs repositories/services/controllers directly
- что станет truth:
...
- `Read switch point`: page constructor/state no longer calls `AppDatabase.instance` or `DiscoveryController(...)`
...
1. перечислить все assembly points inside `DiscoveryPage`
2. вынести graph construction выше UI boundary
3. передавать в страницу только готовые boundaries
4. убрать widget-owned instantiation of controller/repositories/services
5. прогнать UI smoke tests
6. зафиксировать deletion proof для page-side assembly code
...
- До начала нужны: UI integration smoke tests
- Подтверждают cutover: page entry and feature launch smoke tests
- Hard stop failure:
  - page still constructs low-level dependencies after claimed cutover

## 11. Completion criteria

- `DiscoveryPage` no longer constructs repositories/services/controller
- page remains able to open related feature flows through injected boundaries

## 12. Deletions unlocked

- dependency assembly inside `DiscoveryPage`
- does not yet unlock `LegacyDiscoveryFacade` deletion
