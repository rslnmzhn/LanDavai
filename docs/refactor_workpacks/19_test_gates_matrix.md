...
| `GATE-03` | discovery/files UI smoke and widget tests | `02`, `03`, `04`, `05` | discovery entry flows, files entry flows, history/clipboard launch survivability after UI shell changes (fully covered; includes discovery → files launch, files/viewer entry, remote-share preview/viewer launch, and history populated/open-folder action) | `test/smoke_test.dart`, `test/blocked_entry_flow_regression_test.dart`, `test/files_presentation_import_test.dart`, `test/files_entry_flow_regression_test.dart`, `test/remote_share_viewer_flow_regression_test.dart`, `test/history_entry_flow_regression_test.dart` |
