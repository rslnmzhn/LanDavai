...
- что сейчас является truth:
  ...
  - `known_devices` for alias/trust persistence
- что станет truth:
  - `DeviceRegistry`
...
- `Read switch point`: non-UI discovery logic stops resolving identity through `_devicesByIp` / `_aliasByMac`
...
- `Why it exists`: keep old discovery flows readable while identity authority moves to registry
...
1. зафиксировать registry contract over MAC-normalized device identity
2. перевести device observation writes на registry
3. оставить `_devicesByIp` только как reachability projection
4. сверить old/new identity resolution under migration regression tests
5. запретить новые writes в controller identity fields
6. зафиксировать deletion proof для legacy identity fields
...
- До начала нужны: identity mapping tests, repository contract tests for `known_devices`
- Подтверждают cutover: migration regression comparing old/new identity resolution paths
- Hard stop failure:
  - alias or stable identity no longer follows MAC under IP change

## 11. Completion criteria

- `DeviceRegistry` is the sole identity writer
- `_devicesByIp` is not used as stable identity truth
- new code reads stable identity from registry contract
...
- запрещён dual-write controller maps + registry under production path
