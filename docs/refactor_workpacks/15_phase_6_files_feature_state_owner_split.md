...
- Вынести explorer navigation/filter/sort/selection state из `part`-shared files module в explicit owner.
...
Master plan фиксирует fake decomposition в `file_explorer_*`. Этот slice выделен отдельно, потому что explorer view state — самостоятельный ownership seam и не должен мигрировать вместе с preview cache.

## 4. Legacy owner and target owner

- `Legacy owner`: `file_explorer_*` part graph
- `Target owner`: `Files feature state owner`
- `State seam closed`: explorer navigation/view state vs widget tree/private namespace
...
- что станет projection:
  - explorer tree/list view models
- что станет cache:
...
- `Legacy write path`: user actions mutate page/part state directly
- `Target write path`: user actions dispatch to files feature owner
- `Write switch point`: path/filter/sort/selection stop mutating hidden page state as primary truth
...
1. inventory explorer state currently hidden in `part` namespace
2. move navigation/filter/sort/selection ownership to explicit owner
...
- Hard stop failure:
  - widget behavior still depends on hidden `part` state mutation

## 11. Completion criteria

- explorer navigation/filter/sort/selection have one explicit owner
- files widgets stop treating `part` namespace as source of truth
